...
=== HERO GENERATIONS & CLASSES (AUTHORITATIVE - FROM heroes.json) ===
CRITICAL: Always reference this table. NEVER guess a hero's generation or class.
Format: Name (Class) [Tier]

Gen 1: Bahiti (Mar) B, Charlie (Lan) D, Cloris (Mar) D, Eugene (Inf) D, Gina (Mar) C, Jasser (Mar) B, Jeronimo (Inf) S+, JESSIE (Lan) A, Ling Xue (Lan) B, Lumak Bokan (Inf) B, Molly (Lan) B, Natalia (Inf) A, Patrick (Lan) B, Seo-yoon (Mar) C, SERGEY (Inf) B, Smith (Inf) D, Zinman (Mar) C
Gen 2: Alonso (Mar) A, Flint (Inf) A, Philly (Lan) A
Gen 3: Greg (Mar) A, Logan (Inf) A, Mia (Lan) A
Gen 4: Ahmose (Inf) A, Lynn (Mar) A, Reina (Lan) A
Gen 5: Gwen (Mar) S, Hector (Inf) A, Norah (Lan) S
Gen 6: Renee (Lan) S, Wayne (Mar) A, Wu Ming (Inf) S+
Gen 7: Bradley (Mar) S, Edith (Inf) S, Gordon (Lan) S
Gen 8: Gatot (Inf) S, Hendrik (Mar) S, Sonya (Lan) S
Gen 9: Fred (Lan) A, Magnus (Inf) S+, Xura (Mar) S
Gen 10: Blanchette (Mar) S+, Freya (Lan) S, Gregory (Inf) A
Gen 11: Eleonora (Inf) A, Lloyd (Lan) A, Rufus (Mar) S
Gen 12: Hervor (Inf) S+, Karol (Lan) S, Ligeia (Mar) S
Gen 13: Flora (Lan) S, Gisela (Inf) A, Vulcanus (Mar) S+
Gen 14: Cara (Mar) S, Dominic (Lan) S, Elif (Inf) S+

Class key: Inf=Infantry, Mar=Marksman, Lan=Lancer

KEY HEROES:
- JESSIE (Lan) Gen 1 - Best attack joiner (Stand of Arms skill)
- SERGEY (Inf) Gen 1 - Best garrison joiner (Defenders' Edge skill)
- Jeronimo (Inf) Gen 1 - Top infantry hero
- Wu Ming (Inf) Gen 6 - Strong infantry alternative
- Hector (Inf) Gen 5 - Strong infantry option
- Gatot (Inf) Gen 8 - Top infantry hero
...
Address the player as "Chief." Use in-game terminology.
...
5. Reference the Chief's actual hero data provided below when relevant."""
    ...
    def format_user_data(self, profile, user_heroes: list, heroes_data: dict, inventory: dict = None) -> str:
        ...
        # Profile section - support both ORM objects and DynamoDB dicts
        if isinstance(profile, dict):
            server_age = profile.get('server_age_days', 0)
            furnace = profile.get('furnace_level', 1)
            spending_profile = profile.get('spending_profile', 'f2p')
            is_farm = profile.get('is_farm_account', False)
        else:
        ...
        if not user_heroes:
            ...
            for uh in user_heroes:
                # Support both ORM objects and DynamoDB dicts
                if isinstance(uh, dict):
                    hero_name = uh.get('hero_name', uh.get('name', 'Unknown'))
                    level = uh.get('level', 1)
                    stars = uh.get('stars', 1)
                    exp1 = uh.get('exploration_skill_1_level', 1)
                    exp2 = uh.get('exploration_skill_2_level', 1)
                    exp3 = uh.get('exploration_skill_3_level', 1)
                    exped1 = uh.get('expedition_skill_1_level', 1)
                    exped2 = uh.get('expedition_skill_2_level', 1)
                    exped3 = uh.get('expedition_skill_3_level', 1)
                else:
                    hero_name = uh.hero.name if hasattr(uh, 'hero') else getattr(uh, 'name', 'Unknown')
                ...
                lines.append(f"- {hero_name} [{tier}|{h_class}|Gen{h_gen}] Lv{level} {star_str} Skills: Expl {exp1}/{exp2}/{exp3} Exped {exped1}/{exped2}/{exped3}")
