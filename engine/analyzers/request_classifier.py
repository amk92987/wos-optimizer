...
class RequestClassifier:
    ...
    def classify(self, question: str) -> ClassifiedRequest:
        ...
        # Default behavior based on question length
        word_count = len(question.split())

        if word_count < 10:
            # Short questions likely can be answered by rules
            return ClassifiedRequest(
                request_type=RequestType.RULES,
                category="general",
                confidence=0.6,
                rule_handler="progression_tracker",
                reason="Short question - trying rules first"
            )
        else:
            # Longer questions may need more context
            return ClassifiedRequest(
                request_type=RequestType.HYBRID,
                category="complex",
                confidence=0.5,
                rule_handler="progression_tracker",
                reason="Complex question - try rules first, then AI if needed"
            )
